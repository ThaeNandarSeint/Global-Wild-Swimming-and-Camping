...
if (empty($admin_id)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($_POST['title'])) {
            ...
            $insert_swimming_site = mysqli_query($mysqli, "INSERT INTO `tbl_swimming_sites`(title, location, maximum_guest_count, price, number_of_tents, width, description, allow_campfires, allow_pets, available_wifi, available_electricity, has_cooking_equipments, can_bike, can_fish, can_sail, can_hike, can_ride_horse, publish_by) 
            VALUES('$title', '$location', '$maximum_guest_count', '$price', '$number_of_tents', '$width', '$description', '$allow_campfires', '$allow_pets',	'$available_wifi', '$available_electricity', '$has_cooking_equipments', '$can_bike', '$can_fish', '$can_sail', '$can_hike', '$can_ride_horse', 1)") or die('query failed');

            if ($insert_swimming_site) {
                $swimming_site_id = mysqli_insert_id($mysqli);
                ...
                for ($i = 0; $i < count($_FILES['images']['tmp_name']); $i++) {
                    ...
                    $insert_swimming_site_images = mysqli_query($mysqli, "INSERT INTO `tbl_swimming_site_images`(url, name, swimming_site_id) VALUES('$image_url', '$image_name', '$swimming_site_id')") or die('query failed');
                }
...
<head>
    ...
    <title>GWSC | ADD NEW SWIMMING SITE</title>
