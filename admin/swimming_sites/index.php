...
if (empty($admin_id)) {
    ...
    $select = mysqli_query($mysqli, "SELECT * FROM tbl_swimming_sites");

    if (isset($_GET['publish'])) {
        $id = $_GET['publish'];
        mysqli_query($mysqli, "UPDATE tbl_swimming_sites SET is_published=1 WHERE id = $id");
        $messages[] = 'Published!';
        header('location:index.php');
    } else if (isset($_GET['unpublish'])) {
        $id = $_GET['unpublish'];
        mysqli_query($mysqli, "UPDATE tbl_swimming_sites SET is_published=0 WHERE id = $id");
...
<head>
    ...
    <title>GWSC | SWIMMING SITES</title>
