...
if (empty($admin_id)) {
    ...
    if (isset($_GET['id'])) {
        ...
        $get_pitch = mysqli_query($mysqli, "SELECT * FROM tbl_pitches WHERE id = $id");
        $pitch = mysqli_fetch_assoc($get_pitch);

        $old_pitch_type_id = $pitch['pitch_type_id'];

        $get_pitch_type = mysqli_query($mysqli, "SELECT * FROM tbl_pitch_types WHERE id = $old_pitch_type_id");
        $pitch_type = mysqli_fetch_assoc($get_pitch_type);

        $get_pitch_images = mysqli_query($mysqli, "SELECT * FROM tbl_pitch_images WHERE pitch_id = $id");
        ...
        if (isset($_POST['submit'])) {
            ...
            if (empty($_POST['title'])) {
                ...
                $update_pitch = mysqli_query($mysqli, "UPDATE tbl_pitches SET 
                title='$title', 
                location='$location', 
            ...
            WHERE id = $id") or die('query failed');
                ...
                if ($update_pitch) {
                    $get_pitch_url = mysqli_query($mysqli, "SELECT url FROM tbl_pitch_images WHERE pitch_id = $id");

                    mysqli_query($mysqli, "DELETE FROM tbl_pitch_images WHERE pitch_id = $id") or die('query failed');

                    while ($pitch_images = mysqli_fetch_assoc($get_pitch_url)) {
                    ...
                    for ($i = 0; $i < count($_FILES['images']['tmp_name']); $i++) {
                        ...
                        mysqli_query($mysqli, "INSERT INTO `tbl_pitch_images`(url, name, pitch_id) VALUES('$image_url', '$image_name', $id)") or die('query failed');
                    }
        ...
        <div>
            ...
            <div class="form_container">
                ...
                <form method="post" action="" enctype="multipart/form-data">
                    ...
                    <div class="text_field_container">
                        <div class="text_field">
                            <input type="number" name="number_of_sites" value="<?php echo $pitch['number_of_sites']; ?>">
                            <span></span>
                    ...
                    <div class="text_field_container">
                        <div class="text_field">
                            <input type="number" name="number_of_rvs" value="<?php echo $pitch['number_of_rvs']; ?>">
                            <span></span>
                    ...
                    <div class="select_wrapper">
                        ...
                        <div class="selected">
                            <?php if (isset($pitch['pitch_type_id'])) {
                                echo $pitch_type['title'];
                            } else {
                                echo "Select Video Category";
                            } ?>
                        </div>
                        <input type="text" hidden name="pitch_type_id" value="<?php echo $pitch['pitch_type_id'] ?>">

                        <div class="select_search_box">
                            <input type="text" placeholder="Start Typing..." onkeyup="onSearch(this);" />
                        </div>
                    </div>
