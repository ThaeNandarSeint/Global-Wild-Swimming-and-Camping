...
if (!isset($_SESSION['customer_id']) && !isset($_SESSION['admin_id']) && !isset($_SESSION['superadmin_id'])) {
    ...
    if (isset($_GET['id'])) {
        ...
        // pitch
        $get_pitch = mysqli_query($mysqli, "SELECT * FROM tbl_swimming_sites WHERE id = $swimming_site_id");
        $pitch = mysqli_fetch_assoc($get_pitch);
        $get_pitch_images = mysqli_query($mysqli, "SELECT * FROM tbl_swimming_site_images WHERE swimming_site_id = $swimming_site_id");

        // increase view count
        $view_count = $pitch['view_count'] + 1;
        mysqli_query($mysqli, "UPDATE tbl_swimming_sites SET view_count = $view_count WHERE id = $swimming_site_id");

        // first pitch image
        $get_pitch_first_image = mysqli_query($mysqli, "SELECT * FROM tbl_swimming_site_images WHERE swimming_site_id = $swimming_site_id LIMIT 1");
        ...
        if (isset($_POST['submit'])) {
            if (!isset($customer_id)) {
                ...
                $insert_review = mysqli_query($mysqli, "INSERT INTO `tbl_swimming_site_reviews`(description, rating, publish_by, swimming_site_id) VALUES('$description', '$rating', $customer_id, $swimming_site_id)") or die('query failed');
        ...
        if (isset($_POST['booking_submit'])) {
            if (!isset($customer_id)) {
                ...
                $insert_pitch_booking = mysqli_query($mysqli, "INSERT INTO `tbl_swimming_site_bookings`(swimming_site_id, customer_id, guest_count, start_date, end_date) VALUES('$swimming_site_id', '$customer_id', '$guest_count', '$start_date', '$end_date')") or die('query failed');
...
<body>
    ...
    <section class="body-70">
        <div class="component_wrapper">
            <h1>Swimming Site<span style="color: var(--color-danger);"> Detail</span></h1>
            <?php
            if (isset($message)) {
                ...
                <div class="detail_card_wrapper">
                    ...
                    <div class="detail_left">
                        <div class="detail_main_image">
                            <img src=<?php echo "../uploads/swimming_sites/" . $pitch_first_image['url'] ?> alt="">
                        </div>
